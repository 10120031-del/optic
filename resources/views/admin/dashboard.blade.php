<x-admin-layout title="Dashboard" heading="Overview" :subheading="__('Last :days days', ['days' => $days])">
    ...
    <div class="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <a href="{{ route('admin.orders.index', ['status' => 'pending']) }}" class="panel flex items-center justify-between gap-3 p-5 transition-colors hover:border-ink">
            <div>
        ...
        <a href="{{ route('admin.returns.index') }}" class="panel flex items-center justify-between gap-3 p-5 transition-colors hover:border-ink">
            <div>
        ...
        <a href="{{ route('admin.reviews.index') }}" class="panel flex items-center justify-between gap-3 p-5 transition-colors hover:border-ink">
            <div>
    ...
    @if (auth()->user()?->isOwner())
        <div class="mb-10">
            <a href="{{ route('admin.users.index') }}" class="panel flex flex-wrap items-center justify-between gap-x-6 gap-y-3 p-5 transition-colors hover:border-ink">
                <div>
                ...
                <div class="flex flex-wrap gap-x-6 gap-y-1 font-mono text-xs text-ink-faint">
    ...
    {{-- Revenue --}}
    <div class="panel mb-10 p-5 sm:p-6">
        <div class="mb-6 flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
            <div>
                <p class="eyebrow">{{ __('Revenue') }}</p>
                <p class="mt-1 font-display text-2xl font-semibold text-ink sm:text-3xl">${{ number_format($totalRevenue, 2) }}</p>
            </div>
        ...
        @else
            ...
            <div class="flex h-32 items-end gap-px sm:gap-1">
    ...
    <div class="mb-10 grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div class="panel p-5 sm:p-6">
            ...
            @else
                ...
                <div class="flex h-28 items-end gap-1 sm:gap-2">
                ...
                <div class="space-y-3">
                    @foreach ($topFramesBySales as $row)
                        <div class="flex items-baseline justify-between gap-3 text-sm">
                            <span class="min-w-0 truncate text-ink">{{ $row->frame_name }}</span>
                            <span class="shrink-0 whitespace-nowrap font-mono text-xs text-ink-soft">{{ $row->units_sold }} {{ __('units') }} &middot; ${{ number_format($row->revenue, 2) }}</span>
                        </div>
                    @endforeach
                </div>
            @endif
            ...
            @else
                <div class="space-y-3">
                    @foreach ($mostViewedFrames as $row)
                        <div class="flex items-baseline justify-between gap-3 text-sm">
                            <span class="min-w-0 truncate text-ink">{{ $row['name'] }}</span>
                            <span class="shrink-0 whitespace-nowrap font-mono text-xs text-ink-soft">{{ $row['views'] }} {{ __('views') }}</span>
                        </div>
                    @endforeach
                </div>
            @endif
            ...
            @else
                <div class="space-y-3">
                    @foreach ($lowStockFrames as $frame)
                        <a href="{{ route('admin.frames.edit', $frame) }}" class="flex items-center justify-between gap-3 text-sm hover:text-ink">
                            <span class="min-w-0 truncate text-ink-soft hover:text-ink">{{ $frame->name }}</span>
                            <span class="shrink-0 badge-{{ $frame->stock === 0 ? 'danger' : 'warn' }}">{{ $frame->stock }} {{ __('left') }}</span>
                        </a>
                    @endforeach
                </div>
            @endif
            ...
            @else
                <div class="space-y-3">
                    @foreach ($lowStockContactLenses as $lens)
                        <a href="{{ route('admin.contact-lenses.edit', $lens) }}" class="flex items-center justify-between gap-3 text-sm hover:text-ink">
                            <span class="min-w-0 truncate text-ink-soft hover:text-ink">{{ $lens->name }}</span>
                            <span class="shrink-0 badge-{{ $lens->stock === 0 ? 'danger' : 'warn' }}">{{ $lens->stock }} {{ __('left') }}</span>
                        </a>
                    @endforeach
                </div>
            @endif
